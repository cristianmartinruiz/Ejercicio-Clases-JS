...
//console.log(numbers);
//let dobleNumbers = numbers.map(n => n * 2); 
...
/* 4-Pide al usuario que ingrese un número y verifica si está presente en el array.
*/
...
//const multiplos3 = newArray.filter(n => n % 3 === 0 && n % 5 !== 0);
...
// Filtra los libros que fueron publicados después de 2000 y crea un nuevo array con los títulos de esos libros en mayúsculas.
...
//console.log(librosPost2000)
//console.log(titleUp)


/*9- Ejercicio: Filtrar y transformar datos en un array
*/
...
const ventasElevadas = ventas.filter(ventas => ventas.cantidad > 8);
...
let ingresosTotales = ventas.map (venta => {
        return {
            ...
            ingresoTotal: venta.cantidad * venta.precioUnitario
        };
});   
...
let mayorIngreso = ingresosTotales.reduce((mayorIngreso, venta) => {
    if (venta.ingresoTotal > mayorIngreso.ingresoTotal) {
        return venta;
    }
    return mayorIngreso;
}, ingresosTotales[0])

console.log(mayorIngreso);
